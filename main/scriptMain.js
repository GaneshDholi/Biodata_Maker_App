...
async function loadTemplates() {
    try {
        const [userDetailsResponse, designResponse, headResponse, bodyResponse] = await Promise.all([
            fetch("https://firebasestorage.googleapis.com/v0/b/milan-4590e.appspot.com/o/carousel%2Fuser_details.json?alt=media&token=1752231170135"),
            fetch("https://firebasestorage.googleapis.com/v0/b/milan-4590e.appspot.com/o/carousel%2Fdesign.json?alt=media&token=1752231169086"),
            fetch("https://firebasestorage.googleapis.com/v0/b/milan-4590e.appspot.com/o/carousel%2Fheader.json?alt=media&token=1752231172365"),
            fetch("https://firebasestorage.googleapis.com/v0/b/milan-4590e.appspot.com/o/carousel%2Fbody.json?alt=media&token=1752231171140")
        ]);

        const userDetailsJsonData = await userDetailsResponse.json();
        designJson = await designResponse.json();
        headJson = await headResponse.json();
        ...
        if (headerDropdown) {
            headerDropdown.style.display = hasHeaderContainer ? "none" : "block";
        }
    } catch (err) {
...
document.querySelectorAll("[data-font-type]").forEach(link => {
...
function setupHeaderDropdownWithLoad(headJson, matchedDesignKey) {
    document.querySelectorAll('[data-header]').forEach(link => {
        link.addEventListener('click', () => {
            ...
            const savedHeader = JSON.parse(sessionStorage.getItem("activeHeader") || "null") || {
                container,
                fields: []
            };
...
function renderFormFields(headerFields = []) {
    ...
    const headerFieldsNormalized = new Set(headerFields.map(h => normalize(h)));
    const skipPreviewIds = [];
    ...
    function refreshHeaderOverlay() {
        const selectedImages = JSON.parse(sessionStorage.getItem("selectedImages") || "[]");
        const firstImage = selectedImages[0];
        const filename = firstImage?.split('/').pop().split('?')[0] || "";

        const matchedDesignKey = Object.keys(designJson).find(key =>
            (designJson[key].image_link || []).some(link => link.includes(filename))
        );

        // ✅ Retrieve currentTemplate from sessionStorage
        const currentTemplate = parseInt(sessionStorage.getItem("currentTemplate") || "0");

        // ✅ Sync updated header to headJson
        const activeHeader = JSON.parse(sessionStorage.getItem("activeHeader") || "null");
        if (activeHeader && headJson) {
            headJson[activeHeader.id] = activeHeader;
        }
    ...
    `;
        ...
        addBtn.addEventListener("click", () => {
            ...
            renderFormFields(headerFields);
            refreshHeaderOverlay(); // ✅ This will reapply the fields visually
        });
        ...
        nextBtn.addEventListener("click", () => {
            ...
            if (!isLast) {
                document.querySelector(`.section-block[data-section="${sectionOrder[sIndex + 1]}"]`).style.display = "block";
            } else {
    ...
    function attachFieldListeners(row, field, idx) {
        ...
        row.querySelector("input.photo-input")?.addEventListener("change", e => {
            ...
            reader.onload = ev => {
                ...
                // Update field value in both template.fields and sessionStorage
                ...
                try {
                    ...
                    if (storedHeader?.fields) {
                        ...
                        if (match) {
                            ...
                            // If style is not set, update it too
...
function initEditModal() {
    ...
    if (!modal || !labelField || !valueField || !saveButton) {
        console.error("❌ Modal elements not found in DOM.");
        return;
    }

    // Helper: Strip tags from value input (for raw field)
    ...
    function formatText(startTag, endTag) {
        ...
        const selected = valueField.value.substring(start, end);
        const before = valueField.value.substring(0, start);
        const after = valueField.value.substring(end);
        valueField.value = before + startTag + selected + endTag + after;
        valueField.setSelectionRange(start + startTag.length, end + startTag.length);
        valueField.focus();
    }

    // Save modal values to actual inputs and field data
    function saveChanges() {
        const updatedLabel = labelField.value;
        const updatedValue = valueField.value;

        if (currentEditingLabelInput && currentEditingValueInput) {
            currentEditingLabelInput.value = updatedLabel;
            currentEditingValueInput.value = stripTags(updatedValue); // plain value
            ...
            if (span) span.innerHTML = updatedValue;
    ...
    // Attach toolbar formatting events
    ...
    // Open modal on click of any `.input-icon-wrapper`
    document.addEventListener("click", (e) => {
        const wrapper = e.target.closest(".input-icon-wrapper");
        if (!wrapper) return;
        ...
        labelField.value = labelInput.value;
        valueField.value = valueInput.value;
